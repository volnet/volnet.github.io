...
去中心化并非比特币独有的特性，在其他技术领域也有出现，互联网本身就是个去中心化的系统，电子邮件也是，它实质上是一种简单邮件传输协议（SMTP），任何人都可以搭建邮件服务器。

比特币如何做到去中心化可以拆解成五个问题：
...
其中前三个都反映了比特币的技术细节，将在本章讨论。

从技术上讲，挖矿过程也是向所有人开放的，但挖矿需要很多资金投入。正因为如此，挖矿领域具有非常高的中心化及挖矿能力集中的倾向。
...
一笔交易发生的时候，其实是向构成比特币网络上的所有节点广播其交易。在分布式系统中达成共识的目标就像上一章中心化的财奴一样，实现相同的作用。

在任何时点，都有以下特点：

1. 每个网络节点都包含一系列区块的总账本；
2. 每个区块都包含已经达成共识的交易清单；
3. 每个网络节点还有未打包进区块的交易（网络节点已经被通知、交易已经发生、但还没有被写进区块的交易）。

达成共识的一个方法是：

1. 约定一个时间，比如每隔10分钟；
2. 每个节点都提议，自己的未被认可的交易成为已经达成共识的区块链后面的下一个区块，然后那些节点会执行一些共识协议，每个节点把自己提议的区块作为输入。
3. 共识协议完成，一个区块会被选作输出值。

这里会遇到的问题有：

1. 有的节点会死机。有些节点是恶意的，想要增加不当交易，其他是诚实的。
2. 被选出的区块可能未包含所有正当交易——它可以等待下一次机会。
3. 点对点网络是不完美的，并非所有对应的点是两两相连的，互联网链路的不良可能会造成网络问题。
4. 由于交易信息分布在整个互联网上，信息传递会有严重延迟。
...
1. 不可能达成共识：
    - 拜占庭将军问题：叛徒数量超过1/3时，这个难题将无法克服。
    - Fisher-Lynch-Paterson不可能结果：在一定的条件下（包括节点行为具有确定性特征），甚至在有一个缺陷的过程中，达成共识都是不可能的。
2. 可能达成共识：
    - Paxos算法协议：在一些妥协下（在小概率条件下，该协议会死机卡住，从而无法继续运行），能做到不产生不一致的结果。

#### 打破传统上的假设

传统理论是针对分布式数据库做的，但是到比特币这个特定场景下，比特币打破了经典模型里的两个假设：

1. 引进了奖励的理念：因为比特币自身是货币，所以能够使用奖励让节点变得诚实。
2. 包含随机性：它不再纠结于规定共识的起点与终点。相反，共识是通过一段较长的时间而达成的，在实际系统中，达成共识大约需要一个小时左右。但即使在一个小时以后，节点们也无法确定哪一个交易块应该进入总账本。但随着时间的流逝，我们对某一个块的认识与最终总体共识想吻合的概率将越来越大，观点出现分歧的概率按指数级下降。
