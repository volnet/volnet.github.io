...
> 这是一本面向初学者的书，甚至不要求学习者拥有编程基础。
>
> 立足于Python3，对语言的基本特征和应用进行展示。
>
> 本笔记包含了这本书关于Python语言的大多数细节，可以作为在线版的Python内容查询。建议收藏后反复使用。
...
双字符串的字符串组成的列表：（如果字符中包含3个字母，则会出现异常，长度必须为2）
...
双字符的字符串组成的元组：（如果字符中包含3个字母，则会出现异常，长度必须为2）
