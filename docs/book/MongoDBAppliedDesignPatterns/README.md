...
> 这不是一本讲MongoDB结构、管理和命令行的书，它把注意力从MongoDB本身转移到了程序设计层面。

> 作者通过对不同类型的应用程序进行实践，列举了各个例子中的难点，并指出了在设计这些系统时，应该采用的思维模式，以及更应该关注哪些点。

> 对于习惯了关系型数据库的用户来说，这本书很好地展示了MongoDB在设计方面的异同点。

> 整理这本书的笔记，不太容易，因为作者用了很多代码来表达观点，而笔记本身又尽量避免引入代码，所以在笔记中有表达不周的地方，还请翻阅原著。

> 这本书对于想要了解非关系型数据库程序设计的用户非常有用，建议阅读。

前言
------------------

本书可提供用户需要的MongoDB特性和解决商业应用上的问题。本书重点关注MongoDB具体实现中遇到的实践性问题，特别适合那些希望利用MongoDB来应对特定扩展性问题的开发者阅读。
...
MongoDB既允许采用嵌入的方式存储文档，也可以按照标准化的方式（引用）存储文档：
...
- 采用引用的方式，会导致查询过程较复杂
- 采用嵌入的方式，会导致更新的时候需要更新所有嵌入了该对象的文档的信息。
- 建议：采用折中的方式，嵌入`_id`的列表，而不是嵌入完整的文档。
...
MongoDB有一个主要的缺点是缺乏强制模式，这就是存储效率。主要表现在每一个文档都要存储字段的名字和类型。
...
通过调整MongoDB的Write Concern，可以在存入数据库的重要性和插入速度之间进行平衡。通过`db.events.insert(event, w=1)`、`db.events.insert(event, w=2)`、`db.events.insert(event, j=True)`、`db.events.insert(event, j=True, w=2)`等选项来控制这个平衡。
...
在上述情形下，最佳的方法是使用代表性的数据来测试，并多使用explain()方法。如果发现MongoDB查询优化器正在选择一种不好的索引方式（比如在进行巨大的内存里排序时，选择降低条目扫描的数量），也可以使用hint()方法告诉如何选择索引。
...
作者举例了各种查询的例子，同时也提到了全文检索，为了确保效率，对这种更新频率不高的类型，建议对所有事物进行索引，因为我们无法预测用户会搜索哪个字段。

#### 分片需要考虑事项

合理的分片有助于改善性能。

对于那些难以切分的数据，可以考虑在查询的时候指定特定的副本集，从而获得额外的读性能。
...
- 将购物车状态设置为complete
- 移除product集合中来自任何carted属性对该购物车的所有引用。
...
基于以下原则：目标是最小化用于显示指定网站所需要载入的文档数量，甚至是以复杂更新为代价。

### 模式设计

这里提出的解决方案，将使用两个主要的独立的集合，以及三个非独立的集合来存储用户配置数据和帖子。

#### 独立集合

- social.user：存储特定用户的社交图谱信息
    - 用户id值，使用base64后的值，使得可以在一个查询或者更新中深入文档内部。
    - 在追随者和收集的圈子内双向地存储社交图谱。
    - 包含了一个阻止列表（blocked），包含了一个提供给那些帖子不出现在用户墙或新闻feed上的发帖者的用户ID数组。
- social.post：存储各种类型的帖子
    - 仅保留部分作者信息，以及包含作者配置文件的链接，而不包含所有的信息。
    - 某个帖子的属性，是由圈子的属性控制的。可以设置成公开或者仅圈子可以阅读。
    - 将不同类型的帖子独立分离为子文档，有助于确定文档哪些部分对所有帖子都是固定的，哪些是可变的。

#### 非独立集合

- social.wall：用于显示一个“帖子墙”（类似Facebook的个人页）
    - 每一个帖子都出现一些简短的评论，控制可显示的最大数量，使用查看更多的功能，再单独查询social.post集合。
    - 每个social.user文档有多个social.wall文档，每一个月一个。可以在界面上优先显示最近的帖子，并在需要更早的数据的时候，再次查询。
    - 仅包含用户信息的最小值
    - 存储每个帖子的评论数，
- social.news：followers（粉丝）发布的新闻。

### 操作

#### 查看新闻feed或者帖子墙

实现`get_posts`获取用户墙或新闻feed的所有帖子，按照日期倒序排列。

再实现几个过滤器用于筛选：

- `visible_on_own_wall`：决定在用户查看自身帖子墙时是否显示。
- `visible_on_own_incoming`：查看所有直接发给用户，而不考虑该帖子是否被该用户追随的所有帖子。
- `visible_post`：当查看一个新闻feed或者其他用户的帖子墙时，基于帖子circles属性，是否可见。

#### 在帖子上发布评论

发表评论的时候，要同时将评论更新到`social.post`、`social.wall`、`social.news`中。

同时为了保持之前在帖子墙和新闻页面仅显示固定数量的评论，因此单独实现个轮询的方法，来清理`social.wall`、`social.news`中超过指定数量的评论。

#### 发布新帖子

发布新帖子和发布评论类似，都需要同时在`social.post`、`social.wall`、`social.news`中更新。

#### 维护社交图谱

将一个用户添加到某个圈子中，不仅需要更新到`social.user`集合中，还需要将用户的帖子更新进来（原书中考虑到该操作过于复杂，未进行示范）。

### 分片

针对`social.user`、`social.wall`、`social.news`，可以使用用户的_id字段进行分片。

针对`social.post`，应该使用post的_id字段。
...
在设计在线游戏的时候，需要存储各类玩家角色，还要存储大量同时玩游戏的玩家数据，这些数据需要读延迟和写延迟都很低，以保证在游戏期间的响应能力。

### 模式设计

#### 角色模式（玩家状态）

- 为了使游戏系统可以直接绘制房间，而不需要再次执行查询数据库来获取房间信息，因此在该文档中也需要放置所有必要的描述房间的信息。
- 装备（armor）和武器（weapons）属性包含了很少关于该物品是否被穿上、携带的信息。因为该信息也会包含在同一个文档的inventory中。
- Inventory包含了物品的细节情况。
...
在游戏需要多个角色之间交互的时候，或者角色与非库存物品之间交互时，需要将location用作记录系统。

### 操作

在大部分的操作中，都可以避免操作物品数据（item）和位置数据（location）。
...
为了将设计的“最小”的数据结构用于数据展示，需要在应用程序服务器中，将数据拼接成目标模板所需要的数据。

（这里略去具体代码）

#### 提取用于显示的角色属性、清单和房间信息

这些信息主要都是单行文档所对应的一级节点的文档，直接取出来即可。

#### 从房间获得一个物品

主要考虑如果允许多个玩家或者非玩家角色捡起物品，就会遇到两个角色同时希望捡起这个物品的问题。为了避免出现该问题，可以使用location集合来决定如何处理。

如果位置信息没有更新，就抛异常，不再进行角色状态中的位置和物品更新，否则进行更新。

#### 从容器中移除物品

假设玩家希望移动某个物品到库存中，就需要更新角色状态和物品状态。

需要注意的一点也是需要保证不同数据结构之间的事务性。
...
因为主要的操作都是基于用户角色状态的，因此直接基于不同的用户`_id`进行分片就好。
