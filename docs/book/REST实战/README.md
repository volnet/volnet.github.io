...
> 这是一本非常好的讲述Web的书，它不是教你构建网页，而是告诉你如何使用Web来构建分布式系统，它循序渐进，由浅入深，把Web的整个世界，娓娓道来。
...
- 边界：消费者和其他人员之间有明显的边界
- 菜单：应该能很清楚地表达咖啡产品和其他可选项，以及我们应该如何更好地设计它们。
...
因为它只是将Web作为传输协议，所以Web架构固有的可伸缩性、可靠性和健壮性，对于基于POX的解决方案来说并不存在。
...
CRUD式服务在Richardson成熟度模型中位于第二级，像Amazon S3的产品就是典型的二级服务。
...
如果创建失败，则返回400状态码作为响应，表示客户端发送了一笔服务器不能理解的订单。此时客户端应该尽量不要重新提交该订单，因为这会导致同样的400响应。
...
契约是运行时在Web上指定的。契约中最后落实元素是一个（或多个）对外公布的入口点URI，这时再向消费者进行广告，使它们可以绑定到服务。
...
从代码的角度，仅实现那些能够将资源迁移到某个状态的方法即可，如上文中的NewOrder，并且该方法的使用传入的HTTP载荷（这里是xml），并返回合适的HTTP载荷（这里是xml）。
...
- 联合内容
- 表述文档和文档类似的结构：很多领域的资源是类似文档这样被结构化的，它们可以直接使用Atom的元数据元素。
- 创建元数据丰富的资源列表
- 在现有的资源表述上添加元数据
- 创建非超媒体内容的目录
...
- 工作提要：包含了所有过去的某个分界点到现在为止发生的事件。（它的URI通常和归档提要类似，只是它一直在变，比如当前工作提要是：http://xxx/2017/03/26 ）
- 归档提要：包含了所有在分界点之前发生的事件。
- 最近事件提要：它不是一个历史提要，它总是最新的。（它的URI通常是一个固定值，它的内容在当前时间点和工作提要一致，但是它始终是类似：http://xxx/latest ）
...
本节分步描述了订单履行的过程，重点在于每次调整订单状态都会使用ETag和If-Match来防止冲突，从而实现了使用AtomPub在多个角色之间的状态转移。具体的过程请参看原书。
...
作者分别站在三个角色的角度，使用Jersey实现了一遍各个角色的代码。

#### OAuth的实践考虑

OAuth2.0涵盖了Web应用、桌面应用、移动电话和家庭设备等方面的特定流程，且不提供对OAuth 1.0的向下兼容。

### 服务的黑客攻击和防御

服务需要考虑的五个重要的安全议题：

1. 拒绝服务：应该设定Content-Type，并限定大小避免耗尽服务器资源，而且不应该返回任何错误给客户端，避免泄露信息。
2. 保密：有一些错误码过于详细，存在被黑客猜对的可能，比如严格地区分了400、401、405等，但是很多时候这种错误对于正确的客户端并没有太大的帮助，那么统一回复400可能更安全。另外URI中的路径信息也可能暴露不同的细节，那么可以尝试将不同的URI用一些UUID隐藏起来，就像是短链接一样。
3. 防御性动作：黑客可能借助服务攻击操作系统获得密码等敏感信息或者造成服务攻击，可以采用一些主流的服务框架，通常都已经进行了良好的防御，也可以准备一些测试脚本用来模拟这些常见的攻击，以便提早发现漏洞，比如安全扫描之类的。
4. 更少即是最好：越多的代码越容易出现安全漏洞，特别是一些预想着可能用到的代码，但实际并没有用到的代码，能不写尽量不写。
5. 深度防御：通过硬件／软件的防火墙，关闭不必要暴露的端口，不要使用root权限来运行web，要防止社会工程攻击。

### 最后的思考

即使在企业内部相对安全的环境也要使用基于Web的做法来保障安全。
...
语义是为了使得不同的程序之间可以通过某种语言互相理解。

### 信息的结构和表述

人们将 **数据(data)** 用 **信息(information)** 的形式进行表述、传播，并解释成不同的 **知识(knowledge)** 。

### 语义网

原书用RDF、OWL来示意描述Restbucks的信息，用SPARQL可以采用类似SQL的语法来查询这些信息。带属性的RDF（RDF in attribute，RDFa）填平了RDF与人类的Web之间的鸿沟，使得这些语法既能够传达表述又能够传达语义信息。

### 微格式

微格式采用现有的XHTML规范来描述信息，是一种草根发起的简化的格式。

### 链接数据和Web

链接数据的目的就是暴露数据和信息，以便计算机（而不是人类）可以使用并进行处理。

鼓励企业和组织使用语义网技术来暴露数据，并且和Web上的其他数据链接起来，就可以形成全球数据图。

### 指导

计算机到计算机的交互中契约和协议非常重要，我们需要共享的是对被交换信息的理解，而不是对被交换信息的处理过程。
...
在2000年，Web Services刚出现的时候，它比DCOM、RMI和CORBA等技术都要更标准，能够实现跨系统互联。

但是WS-*协议栈的规范之间存在着很多不兼容、不一致甚至完全错误的情况，这对于Web Services的发展没有任何帮助。

对于Web Services是好是坏，需要辩证地看待。

### SOAP：全部真相

SOAP定义了XML信封和一个跨网络传输消息的处理模型。

#### SOAP的处理模型

SOAP通过HTTP传输就是SOAP over HTTP，它也可以通过其他的传输协议进行传输。而实际上，它把HTTP作为传输协议。但是在本书中，我们已经习惯了HTTP是一个应用协议，而不单纯是传输协议，SOAP将GET、POST、PUT等语义都忽略了，它实质上比HTTP协议更低层次，它没有在所转移的载荷之上强加任何应用的语义，并且将对消息的解释留给接收消息的服务。

#### 制造爱，而不是制造战争

SOAP对Web语义上的错误使用，扰乱了Web作为应用协议的常规使用方式，而且还无法使用各种中间件（如缓存服务器）的优势。

SOAP和HTTP在一些方面是有一定相似性的：

1. 信封：SOAP和HTTP在信封上是一致的，只是格式不太一样。都包含了头信息和消息体。
2. 头信息：SOAP（`<soap:Header>`）和HTTP都可以包含任意数量的头信息。SOAP在头信息内部用WS-Addressing重新定义了目标地址。
3. 消息体：SOAP和HTTP都包含消息体。HTTP携带的是资源的状态表述；在SOAP信封的例子中，携带的是必须由交互的各方进行解释的载荷。
4. 中间组件：SOAP和HTTP都可以在源和目标之间增加中间组件，这些组件（如缓存）可以根据头信息的协议进行处理。
5. 错误：HTTP用状态码来标记服务协议的错误，而SOAP fault虽然也表达错误，但是并没有定义足够的标准化信息，以至于这个机制通常被用来传递系统内的编程错误，导致调用方没有办法按照错误信息的指导进行一些补偿处理。

### WSDL：不过是另一种对象接口定义语言（Object IDL）

WSDL被认为是SOAP的面向消息的处理模型，但实际上它不过只是一个冗长的对象接口定义语言（IDL），它强制Web Services使用并不适宜的RPC风格的参数、返回值和异常。它通常是一种对操作的抽象，而不是对资源的抽象。

WSDL以及它对应的一系列工具，让我们很容易将程序内部的方法及领域模型暴露给服务的外部，使得系统间的耦合性大大增强。同时这些工具还试图隐藏分布式系统设计的复杂性，使得调用者使用起来像是用本地的接口，然而这样的设计思路倒是让系统无法应对高延迟和低可靠性。

紧耦合带来的最大问题是无法应对领域模型的变化，导致代码的修改变得代价高昂。虽然有的框架引入了服务模型的概念来作为系统边界的交互对象，但是因为这些历史问题夹杂在一起，同时WSDL工具所带来的紧耦合仍然存在，导致想要实现一个理想的系统，变得非常困难。

### 两个错误叠加无法得到正确结果

Web中也存在类似WSDL的WADL。它也存在以下问题：

1. 它描述的是Web应用的静态视图，它提供对资源的CRUD操作。
2. WADL工具同样导致紧耦合问题，换句话说，它把服务所提供的资源，直接变成使用者的领域模型。
3. 它没有提供与它进行广告的资源交互的顺序，也就是无法进行下一步操作。
4. 它经常造成元数据的重复。

不过万幸的是，它没有像WSDL那样长足地发展，因此没有那么大的影响力和破坏力。

### 安全的，可靠的，事务性的

#### 安全性

Web Services使用了一套WS-*协议来提供端到端的加密。为了实现安全性，提供了一堆专家级的协议方案，这些方案很聪明，但是复杂度很高，使得一般的开发者望而却步，最终可能导致在使用这些协议的时候，容易犯错误从而导致不安全性。

#### Web安全

Web使用了HTTPS提供了端到端的加密，同时也不允许进行中间件缓存等，大大增强了安全性。

使用OpenID和OAuth等简化的安全机制来提供身份验证和资源授权。

#### 可靠的消息传递

Web Services通过WS-ReliableMessage协议来实现可靠性，具体包括消息的传递可靠性，支持以下四种模式：

1. 最多一次：重复的消息将不会被交付，消息会被丢弃。
2. 至少一次：每一个消息都将被交付，但可能会出现重复。
3. 恰好一次：每个消息将被交付一次，只有一次。
4. 按顺序：消息的接收与发送顺序相同。

而Web天生是同步模式的、支持幂等操作、对非幂等操作可以提供[POE方式](https://www.ietf.org/archive/id/draft-nottingham-http-poe-00.txt)等，来支持消息可靠性的四种模式。

#### 事务

Web Services实现的协议，多是两阶段提交协议的变种，这种协议可以实现事务，但是也有一定的局限性，它会降低系统的响应能力。

#### Web中的事务

可以使用类似Web Services的两阶段提交协议来实现Web的事务，但即便如此，多数情况下并不需要在Web中引入事务，因为每个Web请求都已经返回了明确的状态，或成功（200）或者失败（400）。

另一种观点认为，应该把事务放在Web调用的具体实现里，而不需要暴露到外面来。这样也可以保证较小的资源占用。

### Web Services的安魂曲？

Web Services基于SOAP，是一个很成熟的体系，然而它是反模式的（不好的）。越来越多的事情，可以不用Web Services而使用Web来解决。
...
Web可能解决很多问题，但不适用于所有问题，理解它并合理运用它，会有更有益。

### 建造并运行基于Web的服务

如果不是要建造实时的或安全性关键的系统，否则Web都很合适。（意味着这两类不合适？）

使用Web去构建系统，可以获得Web带来的所有好处，包括现成的基础设施、成熟的开发工具、成熟的方法论、测试便利性等。

### 没有度量就没有架构

Web提供了丰富的特性用来构建分布式系统，但是仍然有很多架构设计是需要架构师自行解决的，和现在的任何软件研发相比，并没有太大的差异。

Web可以被用来构建分布式系统，大量的文献也已经证明了，分布式系统的分布式特性不应该被隐藏，隐藏后的系统反倒是带来了更多的麻烦，承认分布式的存在，并且采用合理的方式来解决它们，是更为明智的选择。Web的设计本身就承认了分布式的存在，因此它也更适用于分布式。

Web确实不太擅长低延迟的场景，但其实很多时候并不是非常需要低延迟的场景，可以通过合理地评估业务来估计这些场景的合理性。

对错误的处理也需要Web的设计者提前进行充分的考虑，Web基于状态码的状态表述，是一套非常好的用于处理这些问题的好工具。

### 推销Web

要想在企业中实施Web，需要穿越层层阻碍，有很多的利益相关方会无形中阻碍到这件事。但是值得庆幸的是，成本和风险可以用来评估使用Web的可行性。

关于成本，Web有大量的免费、成熟、广泛的中间件可以使用，而且规范和协议相较于私有协议更容易理解，而且天生被设计用于处理分布式系统，而且没有传统厂商的长期的高昂运维成本，取而代之的是谁都可以轻松掌握的Web运维工作。

关于风险，Web可以用敏捷的方法持续交付，而不需要等到所有功能都完成再进行交互，及早发现风险，规避风险。

### 出发去建造

让我们一起建造充满乐趣、容易测试、在生产环境中部署和管理起来简单直接的Web吧。希望本书提供的经验都能有帮助。