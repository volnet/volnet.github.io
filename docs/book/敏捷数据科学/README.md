...
>
> 也正因为这个本质的区别，很多团队在实施大数据项目的时候，客户、产品经理、数据分析师、大数据工程师，这些不同的专业人员没有很好地协作起来，通常的原因是，有人提出假设，有人实现假设，但是就结束了，然后我们提出了一个新假设，而这个过程中没有最终用户的参与，我们将自以为是的结果给到最终用户，我们得到了一个“没用的应用”。
>
> 敏捷正是一种可以避免这种情况的发生的方法，让团队节约成本，更快找到正确解决方案的方法论。作者将这个过程拆解成了5个部分，即“数据-价值金字塔”，但一个核心就是，从第一层开始，就要建立和用户之间的反馈机制，让他们可以看到数据，并及时给予反馈。
> 
> 关于本书，因为仅有150页的中文版，里面还夹着很多的代码，所以实际篇幅并不长，而且如果不打算研究具体代码实现的话，没有必要阅读原文。其中的一些思想，我整理在下面的笔记中了，供大家借鉴。
...
将预测转化为行动，并将这个功能开发成一个可交互的产品特性。

我们将问题升级为：帮助用户提高邮件被回复的概率。

为了实现这个目标，我们将用户要发送的内容每个单词的回复概率进行统计，并将这些统计结果进行比较，并向用户反馈邮件被回复的概率。

通过以上一系列的设计我们完成了一个简单的闭环，这个闭环就是敏捷的数据开发流程，通过数据-价值金字塔不断和用户产生交互，并最终指引用户行动。整个过程就是让价值不断被看见的过程，每一个环节都是推动数据挖掘项目成功的关键。